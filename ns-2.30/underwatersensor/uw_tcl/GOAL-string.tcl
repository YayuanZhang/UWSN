...
set opt(tr)		"GOAL.tr"	;# trace file
set opt(datafile)	"test.data"
set opt(nam)            "GOAL.nam"  ;# nam file
set opt(adhocRouting)   Vectorbasedforward ;#VBF's routing abiliy is disabled and acts as a dummy one
set opt(width)           100
set opt(interval)        3  ;#10.0
set opt(range)           100    ;#range of each node in meters
set opt(max_burst)	 10
...
$a_($total_number) cmd setTargetAddress 0
...
set opt(stop2) [expr $opt(stop)+1000]


puts "Node $total_number is sending first!!"
$ns_ at 1.33 "$a_($total_number) cbr-start"
$ns_ at $opt(stop).001 "$a_($total_number) stop"

$ns_ at $opt(stop2).002 "$a_(0) terminate"
$ns_ at $opt(stop2).002 "$a_($total_number) terminate"
