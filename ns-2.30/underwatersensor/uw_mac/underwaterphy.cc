...
/*,sense_handler(this)*/
{
	...
	*/
    ...
    Pt_consume_ = 30;  //0.66 1.6 W drained power for transmission
    Pr_consume_ = 1;  //0.395 1.2 W drained power for reception
    P_idle_ = 0.2;//0 0 W drained power for idle
...
UnderwaterPhy::command(int argc, const char*const* argv)
{
	...
	if (argc==2) {
		...
		if (strcasecmp(argv[1], "setTxPower") == 0) {
            ...
            propagation_ = (UnderwaterPropagation*) obj;
			return TCL_OK;
...
{
    //printf("underwaterphy: node %d sendDown\n",node_->address());
	...
	* Sanity Check
	*/
    printf("underwaterphy node %d senddown at %lf\n",node_->address(),NOW);
	assert(initialized());
      ...
      case SENDE:
		  {
			  ...
			  if (start_time >= update_energy_time_) {
                  ...
                  printf("node %d DecrIdleEnergy 1 now %lf update_energy_time_%lf \n",index_,NOW,update_energy_time_);
             ...
             // printf("node %d DecrIdleEnergy 2 %lf \n",index_,forwarding_delay());
			  em()->DecrTxEnergy(txtime-forwarding_delay(), Pt_consume_);
             // printf("node %d DecrTxEnergy %lf \n",index_,Pt_consume_);
   ...
   // printf("underwater phy: node %d, got the upgoing packet  ptytype=%d at %f\n", node_->address(),cmnh->ptype_,NOW);
	...
	if (em()) 
		...
		if (start_time > update_energy_time_) {
            ...
            //  printf("node %d DecrIdleEnergy 5 \n",index_);
...
UnderwaterPhy::power_off()
{
	...
	if (NOW > update_energy_time_) {
		em()->DecrIdleEnergy(NOW-update_energy_time_,
			P_idle_);
        printf("node %d DecrIdleEnergy 3 power_off \n",index_);
		update_energy_time_ = NOW;
	}
...
void UnderwaterPhy::UpdateIdleEnergy()
{
	...
	if (NOW > update_energy_time_ && em()->node_on()) {
		em()-> DecrIdleEnergy(NOW-update_energy_time_,
			P_idle_);
        //printf("node %d DecrIdleEnergy 4 UpdateIdleEnergy \n",index_);
		update_energy_time_ = NOW;
	}
